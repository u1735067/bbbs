...
### Why? 
Because Python relies on it to read filenames correctly. Most Unix filesystems seems to only care about `bytes`, but the [convention is to use UTF-8](https://unix.stackexchange.com/questions/2089/what-charset-encoding-is-used-for-filenames-and-paths-on-linux) and most locales without the charset specifier (`.utf8`) doesn't not use UTF-8, so Python use that charset and fails to decode filenames properly.
