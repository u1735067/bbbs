...
if [ $# -lt 3 ]; then
  ...
  echo "example: BORGW_RESTRICT_PATH=/path/to/repos $0 -s /tmp/local.sock /tmp/remote.sock /root/borg-socat-client.sh"\
    "\"backuped-server -p 22\" /root/borg create ssh://backup-server/./my-repo::{hostname}_{utcnow} paths to backup"
  echo "example: SSH_ARGS=\"-o ProxyCommand=ssh -W %h:%p gateway-server -p 22\" BORGW_RESTRICT_REPOSITORY=/path/to/repos/repo"\
    "$0 -s /tmp/local.sock /tmp/remote.sock /root/borg-socat-client.sh backuped-server /root/borg"\
...
#ssh port can be placed with '"host -p 22"' instead of 'host', which will be passed as multiple args (unlike $SSH_ARGS)
  ...
  borg_serve_cmd=$BORGW_PATH"/borg serve --umask 077"
  ...
  if [ "$1" == '-s' -o "$1" == '--socket' ]; then
    ...
    socat_pid=$!
    print_args ssh -R "$2":"$3" $5 "$SSH_ARGS" "BORG_RSH=\"$4 -s $3\"" "${@:6}"
    [ "$BORGW_DRYRUN" == "1" ] || ssh -R "$2":"$3" $5 "$SSH_ARGS" "BORG_RSH=\"$4 -s $3\"" "${@:6}"
    ...
    socat_pid=$!
    print_args ssh -R "$2":"$host":"$3" $5 "$SSH_ARGS" "BORG_RSH=\"$4 -t $3\"" "${@:6}"
    [ "$BORGW_DRYRUN" == "1" ] || ssh -R "$2":"$host":"$3" $5 "$SSH_ARGS" "BORG_RSH=\"$4 -t $3\"" "${@:6}"
    [ "$BORGW_DRYRUN" == "1" ] || (kill -0 "$socat_pid" && kill $socat_pid)
  else
    echo "unknown method"
  fi
